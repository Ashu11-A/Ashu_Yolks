...
export async function generate (id: string) {
  ...
  for (const job of builds.jobs) {
    ...
    const { type, tag, arch } = JSON.parse(job.name) as { type: string, tag: string, arch: string }
    const text = `| ${tag} | ${arch.includes('amd') ? '✅' : '❌'} | ${arch.includes('arm') ? '✅' : '❌'} | ${sucess ? '✅' : '❌'} | ${time} | ghcr.io/ashu11-a/${type}:${tag}`
